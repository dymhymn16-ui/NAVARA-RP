...
stock HideAuctionTextDrawsForPlayer(playerid)
{
	...
	PlayerInfo[playerid][pIsOpenAuctionMenu] = false;
	PlayerInfo[playerid][pCurrentTake] = PlayerInfo[playerid][pCurrentPage] = 0; 
	return 1;
}

stock ShowVehicleInformation(playerid, vehicle)
{
	new bureau = PlayerInfo[playerid][pBureau];
	...
	
	{
		...
		valstr(string, PlayerInfo[playerid][pCurrentTake] + 1);
